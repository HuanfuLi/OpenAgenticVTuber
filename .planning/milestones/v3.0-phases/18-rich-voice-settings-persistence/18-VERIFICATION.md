...
1. `AUDIO-01`: Provider capability labels are visible for TTS/STT providers.
...
2. `PRIV-01`: Cloud STT providers remain disabled by default and require explicit credentials and consent before audio can be sent.
...
3. `PRIV-02`: Settings, logs, and diagnostics redact STT credentials, reference-audio paths, transcripts, and provider errors where appropriate.
...
4. `PERF-02`: User can inspect TTS/STT latency, timeout, and provider-failure diagnostics without exposing secrets.

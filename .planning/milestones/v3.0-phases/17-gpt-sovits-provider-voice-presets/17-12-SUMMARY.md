...
- Phase 17 is now closed in `17-UAT.md`.
- This plan's live-facing outcomes are recorded as passed: sentence delivery no longer waits on sidecar local playback, renderer audio is queued to avoid overlap, and mouth movement is damped for GPT-SoVITS RMS envelopes.
- Full live retesting remains dependent on the user's external GPT-SoVITS server/model setup, but there are no open Phase 17 UAT blockers.

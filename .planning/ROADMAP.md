...
- 🚧 **v2.0 Plugin + Animation Control** — Phases 8, 6, 7, 9, 10 in progress.
...
**Plans**: ~2 plans (TBD at /gsd:plan-phase 6)

Plans:
- [ ] 06-01-PLAN.md (TBD) — **Plumbing-week sub-phase**: `BodyMotionPlugin` ABC + `PluginManifest` pydantic + `RigCapabilities` contract + manifest loader with reserved-name guard + supervised async-gen task with circuit breaker + safety-clamp pass at compositor → renderer boundary + coalescing rate-limiter + `PluginAdapter(TickDriver)` + `IntentDriver` deletion + system-prompt assembly with action-code section + side-by-side §14 SC baseline harness (captures milestone-1 baselines for Phase 10 re-run)
- [ ] 06-02-PLAN.md (TBD) — Default plugin port (absorbs `IntentDriver` + `compositor/body_sway/*` logic; OLVT 8-emotion vocabulary; rig-adaptive at `on_load`; verifies milestone-1 `[joy]` 300ms blend within tolerance)
...
- [ ] 08-04-PLAN.md — Gap closure: preserve OLVT emotionMap as default-plugin action bindings, expose bindings through AvatarOverrides/RigCapabilities/schema/contracts, produce Teto native-dialog dogfood evidence, and persist VTS smoke evidence — Wave 4 (IMP-05, IMP-08, IMP-09, IMP-10, ARCH-02)
...
**Goal**: A user opens the HUD, sees a scrollable list of all writable rig params populated from `RigCapabilities`, drags any slider, watches the lock auto-engage, releases (sees the lock release), and reads an override-badge on `MouthOpenY` when the lipsync system primitive overrides the user lock. HUD-mode IPC channel is inactive when HUD is closed (preserves AVT-01's "renderer never sees 60 Hz traffic" rule for non-HUD operation).
**Depends on**: Phase 6 (`RigCapabilities` contract; plugin output flowing through compositor) + Phase 7 (dispatch routes — HUD lock must integrate cleanly with variant/event writes) + Phase 8 (`RigCapabilities` populated from import-time introspection)
**Requirements**: HUD-01, HUD-02, HUD-03, HUD-04, HUD-05, HUD-06, HUD-07, HUD-08
**Success Criteria** (what must be TRUE):
  1. User opens the HUD route; renderer fetches `GET /admin/rig-capabilities`, populates a scrollable list of all writable param IDs with name / value display / slider / lock toggle; sidecar opens `/hud/ws` only on mount, closes on unmount (no 15 Hz traffic when HUD closed)
  2. User drags any slider; lock auto-engages renderer-side optimistically; `set-lock(param_id, value)` over `/hud/ws` reaches the sidecar; sidecar confirms within the next 15 Hz frame; the locked param value persists in `compositor.lock_state` (sidecar single-source-of-truth) and the compositor merge applies the lock LAST in merge order
  3. User locks `MouthOpenY` while TTS is playing; lipsync system primitive overrides the lock (speech without mouth movement looks broken); HUD row surfaces an override-badge with the cause ("lipsync overriding") so the user understands why the lock didn't stick
  4. User releases (un-clicks) the lock toggle; control returns to the compositor; param resumes participation in the merge
  5. App restart clears all lock state (session-only persistence — locks are a discovery tool, not a persistent preference)
**Plans**: ~2 plans (TBD at /gsd:plan-phase 9)

Plans:
- [ ] 09-01-PLAN.md (TBD) — Sidecar HUD tap (`compositor/hud_tap.py` 15 Hz gate + fanout) + `LockState: dict[str, float]` integration into compositor merge (locks LAST, with system-primitive override list per ARCH-12) + dedicated `/hud/ws` FastAPI endpoint + `GET /admin/rig-capabilities` HTTP fetch endpoint + `HudMessage` contract codegen
- [ ] 09-02-PLAN.md (TBD) — Renderer HUD UI (dedicated React route + `useHudStream` hook + scrollable param list with slider/lock-toggle/value display + optimistic lock on drag + override-badge UX + AppShell / BottomRail wiring + electron-store HUD position persistence)

**UI hint**: yes  <!-- Dedicated React route for slider HUD; multi-row scrollable param list with slider + lock toggle + override-badge UX. -->
...
- **HUD window behavior**: floating + always-on-top recommended; confirm at plan-time
...
**Goal**: The §14 success-criteria ceremony deferred from milestone-1 (SC-01) lands here under the refactored architecture. All six §14 SCs are re-run against the running system, recorded in `.planning/skeleton-verification.md` with PASS / PARTIAL / FAIL verdicts, and the side-by-side comparison harness (built in Phase 6's plumbing-week sub-phase) confirms tolerance-band passes against milestone-1 baselines. Cursor polish is OPTIONAL — the original §14 SC #4 (cursor tracking visible across the desktop) becomes a PARTIAL verdict per VFY-01, with a documented future-direction rationale.
...
**Success Criteria** (what must be TRUE):
  1. `.planning/skeleton-verification.md` is committed with PASS / PARTIAL / FAIL verdicts for all six §14 SCs and concrete observations per SC; SC #2 (`[joy]` smooth fade) and SC #3 (body sway through the full utterance) — the highest-risk regressions because their responsibilities migrated from compositor-internal to default plugin — both record PASS or document the regression
  2. Side-by-side §14 SC comparison harness (built in Phase 6 plumbing-week) replays current behavior against milestone-1 baselines and reports tolerance-band passes (latency ±100ms, param values ±0.05 — defaults; tune at plan-time if baselines indicate)
...
**Open questions to resolve at plan-time:**
- **§14 SC tolerance bands** (VFY-05): defaults are ±100ms latency, ±0.05 param values — tune when Phase 6 baselines land
...
| PLG-07 | 6 | Default plugin ships in-tree (`plugins/default/`); absorbs `IntentDriver` + body-sway logic; OLVT 8-emotion vocabulary |

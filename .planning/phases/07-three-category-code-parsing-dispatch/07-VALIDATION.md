...
| 07-05-01 | 05 | 2 | PARSE-05, PARSE-06 | unit | `cd sidecar && uv run pytest tests/vts/test_variant_state_manager.py tests/vts/test_event_completion_tracker.py -x --no-header` | Task creates tests, including valid `2833ms -> 3.833s` and fallback `missing/zero/oversized -> 10.0s` assertions | pending |
...
| PARSE-06 | 07-01, 07-02, 07-05, 07-06, 07-07 | `tests/avatar/test_extract_vts.py::test_trigger_animation_uses_motion3_duration` proving `Meta.Duration=1.833` maps through to `EventFire.duration_ms == 2833` after the +1000ms blend pad, `tests/vts/test_event_completion_tracker.py` proving valid durations sleep `duration_ms + 1000ms` and missing/zero/oversized fallback sleeps exactly `10000ms`, contract check for EventEntry/Dispatch |

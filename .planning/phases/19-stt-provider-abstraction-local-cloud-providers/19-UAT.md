...
- LIVE UAT PENDING: from a clean app-managed STT cache, download FunASR/SenseVoiceSmall or faster-whisper from Settings and confirm cache state becomes `downloaded` only after real model files exist and provider health no longer reports missing local STT runtime dependencies.

...
- [ ] **HUD-06**: HUD param list excludes any param ID present in `compositor/lock_filter.py:SYSTEM_PRIMITIVE_OVERRIDES`. This makes lock-override conflicts impossible by construction rather than handling them with badge UX. (Was: override-badge UX; superseded 2026-05-08 by Phase 9 discuss-phase decision — if the user cannot meaningfully lock a param, do not expose it in the HUD at all. `lock-rejected` WS message is retained as an ERROR-log channel for code-bug detection but does not surface in UI.)

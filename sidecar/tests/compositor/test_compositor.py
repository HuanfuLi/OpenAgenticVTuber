...
async def test_merge_modes_separates_add_and_set(recording_writer) -> None:
    compositor = Compositor(
        ...
        speech_driver=StubDriver({"ParamMouthOpenY": 0.7}),
    ...
    assert frame.add_params["ParamAngleX"] == pytest.approx(0.2)
    assert frame.add_params["ParamMouthOpenY"] == pytest.approx(0.7)
...
async def test_idle_runs_continuously_when_no_other_driver(recording_writer) -> None:
    compositor = Compositor(
        writer=recording_writer,
        idle_driver=StubDriver({"ParamAngleX": 0.15}),
    ...
    assert all("ParamAngleX" in frame.add_params for frame in recording_writer.frames)

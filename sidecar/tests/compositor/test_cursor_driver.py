...
def test_cursor_inside_dead_zone_returns_zeros():
    ...
    assert out["FaceAngleX"] == 0.0
    assert out["FaceAngleY"] == 0.0
...
def test_driver_eases_back_after_cursor_exits_canvas(monkeypatch):
    ...
    out_in = drv.tick(0.0)
    assert out_in["FaceAngleX"] == HEAD_MAX_DEFLECTION_DEG
    ...
    assert out_leave["FaceAngleX"] == pytest.approx(HEAD_MAX_DEFLECTION_DEG)

    out_mid = drv.tick(0.5 + (EASE_BACK_DURATION_S / 2.0))
    assert out_mid["FaceAngleX"] == pytest.approx(HEAD_MAX_DEFLECTION_DEG * 0.125)
...
def test_driver_re_enters_canvas_during_ease_back(monkeypatch):
    ...
    assert out_back["FaceAngleX"] == -HEAD_MAX_DEFLECTION_DEG
